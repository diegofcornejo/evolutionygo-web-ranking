...
export default  ({ userId, username, points, wins, losses, winRate, position }: Duelist) => {
	const image = `https://ui-avatars.com/api/?name=${username}&background=random&size=128`;
	const rating = 1 + (winRate / 100) * 4;
	return (

<a href={`/duelists/${userId}`} className='card bg-base-300 w-full shadow-xl cursor-pointer max-w-sm hover:bg-neutral transition-all duration-200 ease-in-out'>
